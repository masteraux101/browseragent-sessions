...
const DEFAULT_HUMAN_WAIT_TIMEOUT = 10 * 60 * 1000;

const HIGH_RISK_TASK_RE = /\b(pay|payment|checkout|purchase|buy|transfer|wire|submit order|place order|delete|remove|destroy|drop database|close account|reset account)\b/i;
const HUMAN_BLOCKER_RE = /captcha|verification code|otp|2fa|two-factor|sms code|security code|验证码|短信验证码|人机验证/i;
...
| call_human          | { "question": "...", "reason": "...", "expected_input": "..." } | Ask the human for help/input         |
...
7. Use "call_human" when the step is high-risk (payment/delete/final confirmation), when the page asks for a code that only a human can provide, or when you are stuck after repeated failed attempts on the same page.
8. Think step-by-step. Explain your reasoning briefly before choosing an action.
...
class BrowserAgentLoop {
   ...
   */
  constructor({ llm, maxSteps = MAX_STEPS, notifyFn = null, sendPhotoFn = null, requestHumanInput = null }) {
    this.llm = llm;
  ...
  _countRecentFailuresOnSamePage(actionHistory, url) {
    let count = 0;
    for (let index = actionHistory.length - 1; index >= 0; index--) {
      const entry = actionHistory[index];
      if (!entry || entry.url !== url) break;
      if (entry.action === 'call_human') break;
  ...
  _shouldAutoEscalateToHuman(task, snapshot, actionHistory) {
    const url = snapshot?.url || '';
    const snapshotText = this.elementParser.formatSnapshotForLLM(snapshot);
    const samePageFailures = this._countRecentFailuresOnSamePage(actionHistory, url);
    const lastError = [...actionHistory].reverse().find(item => item && item.error)?.error || '';

    if (samePageFailures >= 3) {
      return {
        shouldEscalate: true,
        reason: `The agent has failed ${samePageFailures} times on the same page without progress.`,
        expectedInput: 'A numbered element to click or a short next-step instruction.',
      };
    }

    if (HUMAN_BLOCKER_RE.test(snapshotText) || HUMAN_BLOCKER_RE.test(lastError)) {
      return {
        shouldEscalate: true,
        reason: 'The page appears to require a human-only verification code or CAPTCHA step.',
        expectedInput: 'The verification code, or a short instruction describing how to proceed.',
      };
    }

    if (HIGH_RISK_TASK_RE.test(task) && /confirm|submit|pay|delete|remove|purchase|checkout/i.test(snapshotText)) {
      return {
        shouldEscalate: true,
        reason: 'The current page looks like a high-risk confirmation step.',
        expectedInput: 'Explicit approval like "continue", "cancel", or the numbered element to use.',
      };
    }

    return { shouldEscalate: false, reason: '', expectedInput: '' };
  }

  _buildHumanPrompt(task, snapshot, reason, expectedInput) {
    const markLines = (snapshot?.interactiveMarks || [])
      .slice(0, 20)
      .map(mark => {
        const label = mark.text || mark.ariaLabel || mark.placeholder || mark.tag;
        return `[${mark.markId}] ${label}`;
      });

    return [
      'Human help needed for browser task.',
      `Task: ${task.slice(0, 400)}`,
      `Reason: ${reason}`,
      expectedInput ? `Expected input: ${expectedInput}` : '',
      snapshot?.url ? `Page: ${snapshot.url}` : '',
      markLines.length > 0 ? `Visible numbered elements:\n${markLines.join('\n')}` : '',
      'Reply with a numbered element, a short instruction, a verification code, or "cancel".',
    ].filter(Boolean).join('\n\n');
  }

  async _requestHumanAssistance(task, snapshot, actionHistory, payload = {}) {
    const reason = payload.reason || 'The browser agent requested human assistance.';
    const expectedInput = payload.expected_input || payload.expectedInput || 'A short instruction or a numbered element.';
    const question = payload.question || this._buildHumanPrompt(task, snapshot, reason, expectedInput);

    if (!this.requestHumanInput) {
      return {
        success: false,
        type: 'human_unavailable',
        result: `Human assistance requested but no human input channel is available. Reason: ${reason}`,
        actionHistory,
        duration: 0,
      };
    }

    const response = await this.requestHumanInput({
      question,
      reason,
      expectedInput,
      screenshotPath: snapshot?.screenshotPath || '',
      snapshotText: snapshot ? this.elementParser.formatSnapshotForLLM(snapshot) : '',
      timeoutMs: DEFAULT_HUMAN_WAIT_TIMEOUT,
      task,
      url: snapshot?.url || '',
    });
    ...
    if (!response || response.timeout) {
      return {
        success: false,
        type: 'human_timeout',
        result: 'Timed out while waiting for human input during browser automation.',
        actionHistory,
        duration: 0,
      };
    }
    ...
    if (response.cancelled) {
      return {
        ...
        result: 'Human operator cancelled the browser task.',
        actionHistory,
        duration: 0,
      };
    }
  ...
  async run(task, context = {}) {
    ...
    try {
      while (stepCount < this.maxSteps) {
        ...
        const autoEscalation = this._shouldAutoEscalateToHuman(task, lastSnapshot, actionHistory);
        if (autoEscalation.shouldEscalate) {
          console.log(`[BrowserAgent] HITL auto-escalation: ${autoEscalation.reason}`);
          actionHistory.push({
            step: stepCount,
            action: 'call_human',
            params: { reason: autoEscalation.reason, expected_input: autoEscalation.expectedInput },
            reasoning: autoEscalation.reason,
            success: true,
            description: `Requested human assistance: ${autoEscalation.reason}`,
            error: null,
            url: lastSnapshot.url,
          });

          const humanReply = await this._requestHumanAssistance(task, lastSnapshot, actionHistory, {
            reason: autoEscalation.reason,
            expectedInput: autoEscalation.expectedInput,
          });

          if (!humanReply.success) {
            finalResult = {
              success: false,
              type: humanReply.type,
              result: humanReply.result,
              actionHistory,
          ...
          actionHistory[actionHistory.length - 1].description += ` Human replied: ${humanReply.text.slice(0, 120)}`;
          continue;
        }

        // ── Step 2: THINK — LLM decides next action ──
        const action = await this._decideAction(task, snapshotText, actionHistory, humanInputs, lastSnapshot.screenshotPath);
        if (!action) {
        ...
        if (action.type === 'call_human') {
          const humanReply = await this._requestHumanAssistance(task, lastSnapshot, actionHistory, action.params || {});
          actionHistory.push({
            step: stepCount,
            action: action.type,
            params: action.params,
            reasoning: action.reasoning,
            success: !!humanReply.success,
            description: humanReply.success
              ? `Human replied: ${humanReply.text.slice(0, 160)}`
              : humanReply.result,
            error: humanReply.success ? null : humanReply.result,
            url: lastSnapshot.url,
          });
  ...
  async _decideAction(task, snapshotText, actionHistory, humanInputs, screenshotPath) {
    ...
    if (humanInputs.length > 0) {
      userPrompt += '\nHUMAN GUIDANCE (recent):\n';
      for (const item of humanInputs.slice(-3)) {
        userPrompt += `  Step ${item.step}: ${item.text.slice(0, 200)}\n`;
      }
      userPrompt += 'Treat the human guidance as authoritative context for the next action.\n';
    }
 ...
 */
function createBrowserTool(llm, notifyFn, sendPhotoFn, requestHumanInputFn) {
  ...
  return tool(async ({ task, user_hints }) => {
    try {
      const browserAgent = new BrowserAgentLoop({
        ...
        notifyFn,
        sendPhotoFn,
        requestHumanInput: requestHumanInputFn,
      });
...
- Escalates to Human-in-the-Loop when the model asks for help, a risky confirmation is detected, or the flow is stuck
