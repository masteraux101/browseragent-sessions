...
function createBuiltinTools(repoStore, llm, notifyFn, memoryStore = null, requestHumanInputFn = null) {
...
CROP_REGION: {"x": <left>, "y": <top>, "width": <width>, "height": <height>}`;
  ...
  if (llm) {
    try {
      ...
      tools.push(createBrowserTool(llm, notifyFn, sendImageToActiveChannel, requestHumanInputFn));
    } catch (e) {
  ...
  if (llm) {
    try {
      ...
      tools.push(createExplorerTool(llm, repoStore, notifyFn, sendImageToActiveChannel, requestHumanInputFn));
    } catch (e) {
...
- browser_task: ReAct browser automation with Playwright. Supports Human-in-the-Loop escalation for risky steps, CAPTCHA/OTP, and stuck flows.
...
10. For direct web interaction on NON-GitHub sites, prefer browser_task. It drives a real browser step-by-step with atomic actions and can pause for Human-in-the-Loop approval/input. Use explore_task when no existing tool fits, when you need custom code generation/execution, or when the browser path must be wrapped in broader reasoning.
...
- browser_task: ReAct browser automation for non-GitHub websites. Supports Human-in-the-Loop escalation for risky steps or human-only verification.
...
async function sendRuntimeMessage(text) {
  const ctx = _runtime.runtimeCtx;
  if (!ctx || !text) return;

  const payload = String(text);
  try {
    if (ctx.upstash) {
      await ctx.upstash.set(`loop:${ctx.loopKey}:outbox`, createResponse(payload));
    } else if (ctx.repoStore) {
      await ctx.repoStore.writeFile(`loop-agent/channel/${ctx.loopKey}.outbox.json`, createResponse(payload), '[loop-agent] Runtime message');
    }
  } catch (e) {
    console.warn(`[Runtime Message] Outbox write failed: ${e.message}`);
  }

  if (_runtime.listener && _runtime.listener.sendMsg) {
    try {
      await _runtime.listener.sendMsg(payload);
      return;
    } catch (e) {
      console.warn(`[Runtime Message] Listener send failed: ${e.message}`);
    }
  }
  ...
  if (!ctx.upstash) {
    try {
      ...
      await sendNotifications(ctx.pushooChannels, '[Loop Agent]', truncated);
    } catch (e) {
      console.warn(`[Runtime Message] Notification send failed: ${e.message}`);
    }
  }
}

function clearPendingHumanInput() {
  const hitl = _runtime.hitl || {};
  if (hitl.timer) clearTimeout(hitl.timer);
  _runtime.hitl = {
    pending: false,
    question: '',
    expectedInput: '',
...
async function requestHumanInput(payload = {}) {
  if (_runtime.hitl?.pending) {
    throw new Error('Another human-input request is already pending.');
  }

  const question = String(payload.question || 'Human input is required.').trim();
  const timeoutMs = Math.max(5_000, Number(payload.timeoutMs) || 10 * 60 * 1000);
  const expectedInput = String(payload.expectedInput || payload.expected_input || '').trim();

  updateTaskProgress({ node: 'execute', stage: 'waiting_for_human_input' });
  appendTaskThought(`Waiting for human input: ${_trimPreview(question, 200)}`);

  if (payload.screenshotPath && sendImageToActiveChannel) {
    try {
      await sendImageToActiveChannel(payload.screenshotPath, '🧭 Browser task needs human guidance');
    } catch (e) {
      console.warn(`[HITL] Failed to send screenshot: ${e.message}`);
    }
  }

  const prompt = [
    '⏸ Browser task paused for human input.',
    question,
    expectedInput ? `Expected input: ${expectedInput}` : '',
    'Reply in this same chat to resume.',
  ].filter(Boolean).join('\n\n');

  await sendRuntimeMessage(prompt);

  return await new Promise((resolve) => {
    const timer = setTimeout(() => {
      clearPendingHumanInput();
      appendTaskIssue('Timed out while waiting for human input.');
      resolve({ timeout: true, text: '', source: 'timeout' });
    }, timeoutMs);
...
async function tryConsumeHumanInput(text, sourceLabel, ackFn = null) {
  if (!_runtime.hitl?.pending || !_runtime.hitl.resolve) return false;

  const inputText = String(text || '').trim();
  const resolver = _runtime.hitl.resolve;
  clearPendingHumanInput();
  updateTaskProgress({ node: 'execute', stage: 'human_input_received' });
  appendTaskThought(`Human input received via ${sourceLabel}: ${_trimPreview(inputText, 160)}`);

  const cancelled = /^\/?(cancel|abort|stop|取消|停止)\b/i.test(inputText);
  resolver({
    text: inputText,
    source: sourceLabel,
    cancelled,
  ...
  if (ackFn) {
    try {
      await ackFn(cancelled ? '🛑 Human cancelled the paused browser task.' : '✅ Human input received. Resuming browser task.');
    } catch { /* best effort */ }
  }
  return true;
}
...
function formatTaskProgressText() {
  ...
  if (_runtime.hitl?.pending) {
    const waitSec = Math.max(0, Math.round((Date.now() - (_runtime.hitl.requestedAt || Date.now())) / 1000));
    lines.push(`Waiting for human input: ${waitSec}s`);
    if (_runtime.hitl.question) lines.push(`Question: ${_trimPreview(_runtime.hitl.question, 220)}`);
    if (_runtime.hitl.expectedInput) lines.push(`Expected: ${_trimPreview(_runtime.hitl.expectedInput, 160)}`);
  }
...
async function createTelegramListener(ctx) {
  ...
  bot.on('text', async (bctx) => {
    ...
    if (_runtime.processing) {
      if (await tryConsumeHumanInput(text, 'telegram', async (ack) => bctx.reply(ack))) {
        return;
      }
...
async function createWecomListener(ctx) {
  ...
  wsClient.on('message.text', async (frame) => {
    ...
    if (_runtime.processing) {
      if (await tryConsumeHumanInput(content, 'wecombot', async (ack) => {
        const streamId = generateReqId('stream');
        await wsClient.replyStream(frame, streamId, ack, true);
      })) {
        return;
      }
      try {
...
function startBrowserPolling(ctx) {
  ...
  const pollOnce = async () => {
    try {
      ...
      if (_runtime.processing) {
        if (await tryConsumeHumanInput(msg.text, 'browser', async (ack) => sendResponse(ack))) {
          _runtime.pollTimer = setTimeout(pollOnce, currentInterval);
          return;
        }
...
async function main() {
  ...
  try {
    ...
    const tools = createBuiltinTools(repoStore, llm, explorerNotifyFn, memoryStore, requestHumanInput);
